...
class HorariosController {
    static async getHorarios(_req, res) {
        try {
            const horarios = await prisma_1.default.horario.findMany({
    ...
    static async getHorarioById(req, res) {
        try {
            const id_horario = parseInt(req.params.id);
            const horario = await prisma_1.default.horario.findUnique({
                where: { id_horario },
                include: { empleado: true }
            });
            if (!horario)
    ...
    static async createHorario(req, res) {
        try {
            ...
            const parseHora = (h) => {
                ...
                if (mins)
                    date.setUTCMinutes(parseInt(mins));
                date.setUTCSeconds(0);
                return date;
            };
            const nuevoHorario = await prisma_1.default.horario.create({
                data: {
    ...
    static async updateHorario(req, res) {
        try {
            ...
            if (hora_inicio)
                updateData.hora_inicio = parseHora(hora_inicio);
            if (hora_fin)
                updateData.hora_fin = parseHora(hora_fin);
            if (req.body.disponible !== undefined)
                updateData.disponible = Boolean(req.body.disponible);
            const horario = await prisma_1.default.horario.update({
                where: { id_horario },
                data: updateData,
                include: { empleado: true }
            });
    ...
    static async deleteHorario(req, res) {
        try {
            const id_horario = parseInt(req.params.id);
            await prisma_1.default.horario.delete({ where: { id_horario } });
            res.status(204).send();
        }
        catch (error) {
