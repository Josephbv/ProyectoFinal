...
class RolesController {
    static async getRoles(_req, res) {
        try {
            ...
            for (const core of coreRolesDef) {
                try {
                    const existing = await prisma_1.default.roles.findMany({
                        where: { nombre_rol: core.nombre }
                    });
                    let mainRol;
                    if (existing.length === 0) {
                        console.log(`[ROLES] Creating missing role: ${core.nombre}`);
                        mainRol = await prisma_1.default.roles.create({
                        ...
                        if (existing.length > 1) {
                            ...
                            for (const dup of duplicates) {
                                await prisma_1.default.usuario.updateMany({ where: { id_rol: dup.id_rol }, data: { id_rol: mainRol.id_rol } });
                                await prisma_1.default.roles_permisos.deleteMany({ where: { id_rol: dup.id_rol } });
                                await prisma_1.default.roles.delete({ where: { id_rol: dup.id_rol } });
                            }
                        }
                    }
                    const currentPerms = await prisma_1.default.roles_permisos.findMany({
                    ...
                    if (JSON.stringify(currentModNames.sort()) !== JSON.stringify(core.modulos.sort())) {
                        ...
                        await prisma_1.default.roles_permisos.deleteMany({ where: { id_rol: mainRol.id_rol } });
                        for (const moduloName of core.modulos) {
                            let permiso = await prisma_1.default.permisos.findFirst({ where: { descripcion: moduloName } });
                            if (!permiso)
                                permiso = await prisma_1.default.permisos.create({ data: { descripcion: moduloName } });
                            await prisma_1.default.roles_permisos.create({
            ...
            // 3. Return final list
            const rolesList = await prisma_1.default.roles.findMany({
    ...
    static async createRole(req, res) {
        try {
            ...
            const existingRol = await prisma_1.default.roles.findFirst({
            ...
            const nuevo = await prisma_1.default.roles.create({
                data: { nombre_rol: nombre, activo: activo ?? true }
            });
            if (Array.isArray(modulos)) {
                for (const mod of modulos) {
                    let p = await prisma_1.default.permisos.findFirst({ where: { descripcion: mod } });
                    if (!p)
                        p = await prisma_1.default.permisos.create({ data: { descripcion: mod } });
                    await prisma_1.default.roles_permisos.create({ data: { id_rol: nuevo.id_rol, id_permiso: p.id_permiso } });
                }
            }
            res.status(201).json({
    ...
    static async updateRole(req, res) {
        try {
            ...
            const { nombre, activo, modulos } = req.body;
            const base = await prisma_1.default.roles.findUnique({ where: { id_rol } });
            if (!base)
            ...
            const updated = await prisma_1.default.roles.update({
                where: { id_rol },
                data: { nombre_rol: nombre, activo: activo }
            });
            if (Array.isArray(modulos)) {
                await prisma_1.default.roles_permisos.deleteMany({ where: { id_rol } });
                for (const mod of modulos) {
                    let p = await prisma_1.default.permisos.findFirst({ where: { descripcion: mod } });
                    if (!p)
                        p = await prisma_1.default.permisos.create({ data: { descripcion: mod } });
                    await prisma_1.default.roles_permisos.create({ data: { id_rol: updated.id_rol, id_permiso: p.id_permiso } });
                }
            }
            res.json({
    ...
    static async deleteRole(req, res) {
        try {
            const id_rol = parseInt(req.params.id);
            const rol = await prisma_1.default.roles.findUnique({
                where: { id_rol },
            ...
            await prisma_1.default.roles_permisos.deleteMany({ where: { id_rol } });
            await prisma_1.default.roles.delete({ where: { id_rol } });
            res.status(204).send();
        }
        catch (error) {
    ...
    static async getRoleByName(req, res) {
        try {
            ...
            let rol = await prisma_1.default.roles.findFirst({
                where: { nombre_rol: { equals: name } },
                include: { roles_permisos: { include: { permiso: true } } }
            });
            if (!rol) {
                rol = await prisma_1.default.roles.findFirst({
            ...
            res.json({ modulos: rol.roles_permisos.map((rp) => rp.permiso?.descripcion || ''), nombre_rol: rol.nombre_rol });
        }
        catch (e) {
